...
final class ProgressBarBubbleView: UIView {
    ...
    // MARK: - Init
    init(bubbleHeight: CGFloat, barHeight: CGFloat, width: CGFloat = 200, position: CGPoint = CGPointZero) {
        
        let newFrame = CGRect(origin: position, size: CGSize(width: width, height: bubbleHeight + barHeight + padding))
    ...
    private func configureConstraints(bubbleHeight: CGFloat, barHeight: CGFloat) {
        ...
        constraints += NSLayoutConstraint.constraintsWithVisualFormat("V:|[bubbleView(bubbleHeight)]-(padding)-[barView(barHeight)]|", options: [], metrics: metrics, views: views)
